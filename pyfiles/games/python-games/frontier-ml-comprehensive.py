...
class MAML(nn.Module):
    ...
    def inner_loop(self, support_x: torch.Tensor, support_y: torch.Tensor,
        ...
        for _ in range(num_steps):
            ...
            for param, grad in zip(task_model.parameters(), grads, strict=False):
    ...
    def outer_loop(self, tasks: list[tuple[torch.Tensor, torch.Tensor]],
        ...
        for (support_x, support_y), (query_x, query_y) in zip(tasks, query_sets, strict=False):
...
class FederatedAveraging:
    ...
    def client_update(self, client_id: int, client_data: torch.Tensor,
        ...
        for (name, param), (_, global_param) in zip(
            ...
            self.global_model.named_parameters(), strict=False
        ):
    ...
    def aggregate_updates(self, client_updates: list[dict[str, torch.Tensor]],
                         client_weights: list[float] | None = None) -> None:
        """
        ...
        for updates, weight in zip(client_updates, client_weights, strict=False):
...
class DifferentiableNAS(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Weighted sum of all operations
        output = sum(w * op(x) for w, op in zip(weights, self.operations, strict=False))
