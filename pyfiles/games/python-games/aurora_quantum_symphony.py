"""
AURORA QUANTUM SYMPHONY 
September 2025
The Northern Lights
Featuring: Solar wind particles, magnetosphere dancing, atmospheric chemistry,
quantum aurora effects, and the beauty of the polar light curtains
Cazzy Aporbo, MS
Architecture: Where space physics meets art. An attempt.
Please use your imaginaion
...
AURORA_PALETTE = {
    'arctic_mint': '#7BFFA3',           # Vivid mint aurora
    'aurora_rose': '#FF6B9D',           # Bright rose lights
    'northern_lavender': '#C77DFF',     # Vivid lavender curtains
    'polar_aqua': '#5DFDCB',            # Electric aqua shimmer
    'ice_crystal': '#E0E0FF',           # Bright ice crystal
    'midnight_navy': '#0A0A1A',         # Deeper polar sky
    'solar_peach': '#FF8A5B',           # Bright solar wind
    'magnetic_mauve': '#B19CD9',        # Stronger magnetic field lines
    'oxygen_jade': '#39FF71',           # Bright oxygen emission green
    'nitrogen_blush': '#FF69B4',        # Vivid nitrogen emission pink
    'cosmic_pearl': '#FFFFFF',          # Pure white cosmic rays
    'twilight_periwinkle': '#7B68EE',   # Deeper twilight blue
    'polar_sage': '#90EE90',            # Brighter sage green aurora
    'stellar_silver': '#C0C0C0',        # Bright silver starlight
    'frozen_lilac': '#DDA0DD',          # Richer frozen lilac
    'whisper_white': '#F8F8FF',         # Bright whisper white
    'aurora_amber': '#FFD700',          # Golden amber glow
    'celestial_cream': '#FFF8DC',       # Rich celestial cream
    'moonbeam_mist': '#F0F8FF',         # Bright moonbeam mist
    'plasma_pink': '#FF1493',           # Electric plasma pink
    'ethereal_emerald': '#00FF7F',      # Bright ethereal emerald
    'dream_dusk': '#9370DB',            # Rich dream dusk purple
    'pristine_powder': '#FFFAFA',       # Pure pristine snow
    'aurora_opal': '#E6E6FA',           # Soft aurora opal
    'deep_space': '#000011',            # Deep space black
    'aurora_electric': '#00FFFF',       # Electric cyan
    'plasma_fire': '#FF4500',           # Orange plasma fire
    'quantum_violet': '#8A2BE2'         # Deep quantum violet
}
...
class AuroraVisualizer:
    ...
    def _style_axes(self):
        ...
        # Main 3D aurora view
        self.ax_aurora.set_facecolor(AURORA_PALETTE['deep_space'])
        ...
        for particle in self.solar_wind.particles:
            ...
            if len(particle.trail) > 1:
                ...
                for i in range(len(trail_points)-1):
                    alpha_trail = (i+1) / len(trail_points) * 0.6
                    self.ax_aurora.plot([trail_points[i,0], trail_points[i+1,0]],
        ...
        if not self.spectrum_data or len(self.spectrum_data) == 0:
            ...
            for i, (bar, intensity) in enumerate(zip(bars, sample_intensities, strict=False)):
                if intensity > 0.5:
                    ...
                    self.ax_spectrum.scatter(glow_x, glow_y, s=intensity*300,
                                           c=sample_colors[i], alpha=0.4, marker='o')
        else:
